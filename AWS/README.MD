AWS
========


Aps
----

* [EC2](./ec2/README.MD)


Documentation
-------------

* [AWS Documentation](https://docs.aws.amazon.com/index.html?nc2=h_ql_doc)
* [AWS Lambda Documentation](https://docs.aws.amazon.com/lambda/?icmpid=docs_homepage_featuredsvcs)
* [Getting Started](https://aws.amazon.com/getting-started/)
* [AWS services by category](https://docs.aws.amazon.com/whitepapers/latest/aws-overview/amazon-web-services-cloud-platform.html)
* [Tools to Build on AWS](https://aws.amazon.com/developer/tools/)

### Getting Started

* [Setting Up Your AWS Environment](https://aws.amazon.com/getting-started/guides/setup-environment/?pg=gs&sec=gtkaws)
* [Getting Started with the AWS Management Console](https://aws.amazon.com/getting-started/hands-on/getting-started-with-aws-management-console/?pg=gs&sec=gtkaws)
* [Managing your AWS account](https://docs.aws.amazon.com/accounts/latest/reference/managing-accounts.html)
* [Get Started with AWS CDK](https://aws.amazon.com/getting-started/guides/setup-cdk/?pg=gs&sec=gtkaws)


* [Setting Up Your AWS Environment](https://aws.amazon.com/getting-started/guides/setup-environment/)
* [Module 3: Setting Up the AWS CLI](https://aws.amazon.com/getting-started/guides/setup-environment/module-three/)

* [Choose Your Web Application Infrastructure](https://aws.amazon.com/getting-started/guides/deploy-webapp-decision/)

* [Login for AWS local development using console credentials](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sign-in.html)
* [Configuring command completion in the AWS CLI](https://docs.aws.amazon.com/cli/v1/userguide/cli-configure-completion.html)


#### Launch first app 

* [AWS Elastic Beanstalk Documentation](https://docs.aws.amazon.com/elastic-beanstalk/?id=docs_gateway/?intClick=gsrc-2021)
* [Amazon Simple Storage Service (S3)](https://aws.amazon.com/s3/)
* [Get Started with AWS CDK](https://aws.amazon.com/getting-started/guides/setup-cdk/)
* [Configuration options for Elastic Beanstalk](https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/command-options-general.html#command-options-general-autoscalingasg)
* [AWS CLI and get a list of all the supported plaftorms.](https://docs.aws.amazon.com/cli/latest/reference/elasticbeanstalk/list-available-solution-stacks.html)
* [Handling NodeJS dependencies in Elastic Beanstalk packages](https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/nodejs-platform-dependencies.html)


* [Find the hands-on tutorials for your AWS needs](https://aws.amazon.com/getting-started/hands-on/?intClick=gsrc-2021&getting-started-all.sort-by=item.additionalFields.sortOrder&getting-started-all.sort-order=asc&awsf.getting-started-category=*all&awsf.getting-started-level=*all&awsf.getting-started-content-type=*all)



### Others

* [Host a Static Website](https://aws.amazon.com/getting-started/hands-on/host-static-website/)
* [Run a Serverless "Hello, World!"](https://aws.amazon.com/getting-started/hands-on/run-serverless-code/)
* [Launch a Windows Virtual Machine](https://aws.amazon.com/getting-started/hands-on/launch-windows-vm/)

* [Installing or updating the latest version of the AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html)


* [Deploy a LAMP Web App on Amazon Lightsail](https://aws.amazon.com/getting-started/guides/deploy-lamp-lightsail/?pg=cloudessentials)


Courses & Certifications
-----------------------

### Coursera

* [Coursera -- AWS courses](https://www.coursera.org/partners/aws)
* [Amazon EC2 Basics](https://www.coursera.org/learn/aws-amazon-ec2-basics)
* [AWS Fundamentals Specialization](https://www.coursera.org/specializations/aws-fundamentals)
* [AWS Fundamentals Lab Series](https://www.coursera.org/learn/aws-fundamentals-lab-series)
* [AWS Cloud Solutions Architect Professional Certificate](https://www.coursera.org/professional-certificates/aws-cloud-solutions-architect#courses)
* [AWS Cloud Practitioner Essentials](https://www.coursera.org/learn/aws-cloud-practitioner-essentials)
* [AWS Cloud Technology Consultant Professional Certificate](https://www.coursera.org/professional-certificates/aws-cloud-technology-consultant)
* [AWS Generative AI Applications Professional Certificate](https://www.coursera.org/professional-certificates/aws-generative-ai-applications)


Videos
------

* [Getting Started With AWS Cloud | Step-by-Step Guide](https://www.youtube.com/watch?v=CjKhQoYeR4Q&t=105s)
* [AWS Networking Basics For Programmers | Hands On](https://www.youtube.com/watch?v=2doSoMN2xvI)


Guide & Areas of Study
-----------------------

* [Amazon Virtual Private Cloud (Amazon VPC)](https://aws.amazon.com/vpc/)


### CDK 

* [AWS CDK Toolkit (cdk command)](https://docs.aws.amazon.com/cdk/v2/guide/cli.html)
* [Getting started with the AWS CDK](https://docs.aws.amazon.com/cdk/v2/guide/getting_started.html)
* [Your first AWS CDK app](https://docs.aws.amazon.com/cdk/v2/guide/hello_world.html)
* [Working with the AWS CDK in Go](https://docs.aws.amazon.com/cdk/v2/guide/work-with-cdk-go.html)

* [aws-samples/aws-cdk-examples](https://github.com/aws-samples/aws-cdk-examples)


Terms & Keywords
----------------




References & Further Reading
----------


* [IAM or IAM Identity Center](https://www.reddit.com/r/aws/comments/14j4wmn/iam_or_iam_identity_center/)


Notes
-----


