VS Code / Cursor
=======


Find references
---------------



Similar to Pycharm "Find usage"





**Cursor-specific features**

* AI-powered search: Use Cmd/Ctrl+K to ask about usages semantically
* Codebase search: Use Cmd/Ctrl+Shift+F for text search across files

**Quick access**

* Right-click a symbol → "Go to References" or "Find All References"
* Or use the keyboard shortcuts above


From:https://stackoverflow.com/a/75908810/13903942

if you wanna change where CodeLens opens references when clicking on the "2 References" link above definitions the setting you wanna change is Preferred Location to "view" instead of "peak":



Shortcut -- Hotkeys
===================


* F5: Launch -- Run file
* ctrl+alt+f5 (custom for python): run current file

* CTRL+SPACE: Show suggestions
* Shift+Alt+F: Format file

* Go to Definition (F12)
* Peek Definition (Alt+F12)
* Find All References: Shift+F12
* Peek References (Alt+Shift+F12)

* [Go (code navigation)](https://code.visualstudio.com/docs/languages/go#_code-navigation)

* F12: Go definition
* Alt+F12: Peek Definition
* Shift+F12: Show ALl refs
* Shift+Alt+H: Show calls from or to a func
* CTRL+F12: GO to implementation



Plugins
-------

* Material Icons
* Gitlens
* Go
* Remote SSH
    * WSL

* better makefile
* even better toml


### Workspace


* [Supercharge Your Workflow: VS Code Multi-Root Workspaces](https://www.youtube.com/watch?v=TQtrSMgkMVM)

- Open workspace config file: CTRL+SHIFT+P -> `Workspaces: Open Workspace Configuration file`


```bash
mkdir ~/vscode-workspaces
cd ~/vscode-workspaces

code  ~/vscode-workspaces/myproject-123.code-workspace
windsurf ~/vscode-workspaces/myproject-123.code-workspace

### wsl
echo $WSL_DISTRO_NAME
code --remote "wsl+$WSL_DISTRO_NAME" ~/vscode-workspaces/myproject-123.code-workspace

# .bashrc
alias codewsl='code --remote "wsl+$WSL_DISTRO_NAME"'
alias windsurfwsl='code --remote "wsl+$WSL_DISTRO_NAME"'
# or reusable function 

codewsl() {
  code --remote "wsl+$WSL_DISTRO_NAME" "$@"
}

# then 
codewsl ~/vscode-workspaces/myproject-123.code-workspace
windsurfwsl ~/vscode-workspaces/myproject-123.code-workspace
```